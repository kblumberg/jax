...
function doneTyping (a) {
    //do something
    var v = $.inArray(a.kind, _timedEvents)>=0 ? a.value[0]/100 : a.value[0]; 
    var ranking = getRanking(a.kind, v, _player.pos);
    // console.log(a.kind+': '+a.value[0]/100+' = '+ranking);
    updatePlayer(_player, a.kind, a.value[0]/100, ranking);
    // _player
    // makeChart()
}

function lineColorFunc(val) {
...
!function( $ ) {
  ...
  var Slider = function(element, options) {
    ...
    this.kind = this.element.data('kind')||'';
  ...
  Slider.prototype = {
    ...
    calculateValue: function() {
      ...
      // _timer = setTimeout(function_name(this), 200); 
      // if (val instanceof Array) updateRange(val, this.kind);
      // else updateRenewal(val);
      return val;
    },
  ...
  $.fn.slider.defaults = {
    ...
    formater: function(value) {
      if ($.inArray(this.kind, _timedEvents)>=0) return (value/100).toFixed(2);
      else if (this.kind=='vert') return value/2;
      return value;
      if (this.kind=='rev')
        return "$"+commaSeparateNumber(value);
      else if (this.kind=='prob')
        return value+"%";
      return value;
    }
  };
